...
int main(int argc, char **argv)
{
	...
	cvtColor(src, src_gray, CV_BGR2BGRA);
	StarFeatureDetector star;
	vector<KeyPoint> keyPoints;
	//FASTX(src_gray, keyPoints, 55, true, FastFeatureDetector::TYPE_5_8);

	//int total = keyPoints.size();
	
	star(src_gray, keyPoints);
	...
	//fast->detect(src_gray, detectKeyPoint);
	//drawKeypoints(src_gray, detectKeyPoint, keyPointImage1, Scalar(0, 0, 255), DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
	//drawKeypoints(src, detectKeyPoint, keyPointImage2, Scalar(0, 0, 255), DrawMatchesFlags::DEFAULT);
	//FastFeatureDetector fast(55);
	//fast.detect(src, keyPoints);
	//drawKeypoints(src, keyPoints, src, Scalar(0, 0, 255), 0);
	imshow("src", src);
	waitKey(0);
	return 0;
}
